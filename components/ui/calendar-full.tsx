...
const OutOfBoundsDay = ({ day }: OutOfBoundsDayProps) => (
    <div className="relative h-full w-full bg-secondary p-2 text-muted-foreground text-xs opacity-50">{day}</div>
)
...
export const CalendarBody = ({ features, children }: CalendarBodyProps) => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        days.push(
            <div
                className="relative flex h-full w-full flex-col gap-1 p-2 text-foreground text-xs"
                key={day}
            >
                <span className="font-medium">{day}</span>
                <div className="flex flex-col gap-1 mt-1">{featuresForDay.slice(0, 3).map(feature => children({ feature }))}</div>
...
export const CalendarItem = memo(({ feature, className, onClick }: CalendarItemProps) => (
    ...
    >
        ...
        />
        <span className="truncate text-[10px] font-medium leading-none">{feature.name}</span>
    </div>
))
