...
export function LinkKanban({ tasks, onTaskUpdate, onTaskClick }: LinkKanbanProps) {
                        ...
                        {col.items.map(task => (
                            <KanbanBoardColumnListItem
                                ...
                                onDropOverListItem={(data, direction) => {
                                    try {
                                        const card = JSON.parse(data) as Task;
                                        const cardIndex = col.items.findIndex(({ id }) => id === task.id);
                                        const baseIndex = direction === 'top' ? cardIndex : cardIndex + 1;
                                    ...
                                    <div className="flex flex-wrap items-center gap-3 mt-2 text-xs text-muted-foreground">
                                        ...
                                        {task.planDate && (
                                            <div className="flex items-center gap-1 text-purple-500">
                                                <Calendar className="w-3 h-3" />
                                                <span>Plan: {format(new Date(task.planDate), "MMM d")}</span>
                                            </div>
                                        )}
