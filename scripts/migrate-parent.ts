...
async function migrate() {
    ...
    for (const row of result) {
        console.log(`  Project ${row.id}: goal_id=${row.goal_id} → parent_id=${row.parent_id}, parent_type=${row.parent_type}`);
    }

    // 3. Drop the old goal_id column
    await sql`ALTER TABLE projects DROP COLUMN IF EXISTS goal_id`;

    console.log("Migration complete! You can now update the schema and run `bun run db:migrate`.");
}
