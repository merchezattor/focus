...
const OutOfBoundsDay = ({ day }: OutOfBoundsDayProps) => (
	<div className="relative h-full w-full bg-secondary p-2 text-muted-foreground text-xs opacity-50">
		{day}
	</div>
);
...
export const CalendarHeader = ({ className }: CalendarHeaderProps) => {
	...
	return (
		<div className={cn("grid grid-cols-7 border-b", className)}>
			{daysData.map((day) => (
				<div
					className="p-3 text-center text-muted-foreground font-medium text-xs uppercase tracking-wide"
					key={day}
				>
					{day}
				</div>
